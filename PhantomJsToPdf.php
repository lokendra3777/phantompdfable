 ...
 * PhantomJsToPdf
 *
 * This class is a slim wrapper around phantomjs.
 *
 ...
 *
 *      $pdf = new PhantomJsToPdf($options);   // Set global PDF options
 ...
 */
class PhantomJsToPdf
{
    protected $bin = '/usr/bin/phantomjs';
    ...
    public function getCommand($filename)
    {
        ...
        $command .= ' --load-images=yes --local-to-remote-url-access=true  '.dirname(__FILE__).'/html2pdf.js  ';
    ...
    public  function getPdfFilename()
    {
        if($this->tmpFile===null)
        {
            //$tmpFile = tempnam($this->getTmpDir(),'tmp_PhantomJsToPdf_');
            
        	
        	$tmpFile = $this->getTmpDir().'/'.rand().'.pdf';
            Yii::log($tmpFile,'error', 'ext.phantompdfable.Pdfable');

            if($this->createPdf($tmpFile)===true){
                $this->tmpFile = $tmpFile;
            
                Yii::log($tmpFile.' created','error', 'ext.phantompdfable.Pdfable');
            
            }else{
            	 Yii::log($tmpFile.' create error','error', 'ext.phantompdfable.Pdfable');
                //return false;
                 $this->tmpFile = $tmpFile;
            }
        }
    ...
    protected function createPdf($fileName)
    {
        $command = $this->getCommand($fileName);
         ...
         Yii::log($command.' created','error', 'ext.phantompdfable.Pdfable');
