...
class Pdfable extends CBehavior
{
    ...
    public function getPdfOptions()
    {
        if(!isset($this->_options['bin']))
            $this->_options['bin'] = '/usr/bin/phantomjs';
    ...
    public function renderPdf($view, $data=array(), $options=array(), $filename=null)
    {
        ...
        {
            Yii::log("Could not create PDF for view $view",'error','ext.phantompdfable.Pdfable');
            Yii::log($pdf->getError(),'error', 'ext.phantompdfable.Pdfable');
            YII_DEBUG && Yii::trace($pdf->getError(), 'ext.phantompdfable.Pdfable');
        }
    }
    
 public function renderUrl2Pdf($url, $data=array(), $options=array(), $filename=null)
    {
        $pdf = $this->createPdf();
        $pdf->renderUrl($url, $data, $options);
    ...
    {
        Yii::import('ext.phantompdfable.PdfFile');
